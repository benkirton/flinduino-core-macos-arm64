...
pack() {  # pack <staged-dir-name> <archive-name>
    ...
    [ "$sz" -gt 10000 ] || { echo "error: $name is only $sz bytes - is the source a symlink?" >&2; exit 1; }
    sha=$(shasum -a 256 "$OUT/$name" | awk '{print $1}')
    echo "$name $sz $sha"
}

echo "==> staging toolchain"
cp -RL "$TC" "$OUT/stage/mipsel-pic32-elf-10.2.0"
...
cp -RL "$HERE/pic32/." "$OUT/stage/pic32-$VERSION/"
