# flinduino-core-macos-arm64

> **While I _facilitated_ this project, the work was completed almost entirely
> by Claude Code and I do not claim to understand it beyond the very basics.**
...
1. **Arduino IDE → Settings → Additional Boards Manager URLs**, add:
   ...
   ```
   https://raw.githubusercontent.com/benkirton/flinduino-core-macos-arm64/main/package_flinduino_arm64_index.json
   ```

2. **Tools → Board → Boards Manager**, search `Flinduino`, install
   **Flinduino**.

3. **Tools → Board → Flinduino → Flinduino**, then **Tools → Port** and
...
```bash
git clone https://github.com/benkirton/flinduino-core-macos-arm64
cd flinduino-core-macos-arm64
...
Restart the IDE. It will show as `Flinduino (in Sketchbook)` — the
...
### With arduino-cli directly

If you installed via Boards Manager, `arduino-cli` shares the IDE's data
directory and can already see the platform. Add the index URL to its own config
so it can also update and reinstall:

```bash
arduino-cli config add board_manager.additional_urls \
  https://raw.githubusercontent.com/benkirton/flinduino-core-macos-arm64/main/package_flinduino_arm64_index.json
arduino-cli core update-index
arduino-cli core install flinduino-arm64:pic32     # if not already installed

arduino-cli compile -b flinduino-arm64:pic32:Flinduino MySketch
arduino-cli upload  -b flinduino-arm64:pic32:Flinduino -p /dev/cu.usbmodemXXXX MySketch
```

Use `flinduino-arm64:pic32:FlinduinoBB` for the Baseboard.

Note that `arduino-cli upload` does **not** run the flash-layout check described
under [Safety](#safety). `bin/flinduino` does.

### With the bundled wrapper

Works from a source checkout without installing anything, and checks the flash
layout before every upload:
...
```bash
tools/make-release.sh https://github.com/benkirton/flinduino-core-macos-arm64/releases/download/v1.0.0 1.0.0
```
...
Bug reports are most useful with the output of `bin/flinduino doctor` and
