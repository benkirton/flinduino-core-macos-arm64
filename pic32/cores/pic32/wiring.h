...
/*
 * _SKIP_STDLIB_ABS below is a Microchip newlib hook that suppressed its
 * declaration of int abs(int). Mainline newlib has no such hook, so that
 * declaration always happens - and if it happens after the abs() macro below is
 * defined, "int abs (int);" expands to garbage and every C++ translation unit
 * that reaches <cstdlib> fails ("'abs' has not been declared in '::'").
 *
 * Pulling the standard headers in here, before the macro exists, means they
 * parse cleanly once; their include guards make every later inclusion a no-op.
 */
#ifdef __cplusplus
#include <cstdlib>
#include <cmath>
