 ...
 */
 ...
 // Changed time out to 1000 as the Flinduino is only 3V3 and so the Capacitor is only charged
 // to 3V3 and so the time taken to discharge is not as high as if it was charged to 5V
...
#elseif defined(__PIC32MX__)
	// Flinduino
  #define ZUMO_SENSOR_ARRAY_DEFAULT_EMITTER_PIN QTR_NO_EMITTER_PIN
...
class ZumoReflectanceSensorArray : public QTRSensorsRC
{
   ...
   * settings as given.
   */
  ZumoReflectanceSensorArray(unsigned char * pins, unsigned char numSensors, unsigned int timeout = 1000,
  ...
  void init(unsigned char emitterPin = ZUMO_SENSOR_ARRAY_DEFAULT_EMITTER_PIN)
  {
    ...
    QTRSensorsRC::init(sensorPins, sizeof(sensorPins), 1000, emitterPin);
  }
   ...
   *
   */
  void init(unsigned char * pins, unsigned char numSensors, unsigned int timeout = 1000,
    unsigned char emitterPin = ZUMO_SENSOR_ARRAY_DEFAULT_EMITTER_PIN)
  {
    QTRSensorsRC::init(pins, numSensors, timeout, emitterPin);
  }
