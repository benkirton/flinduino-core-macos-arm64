...
void QTRSensors::readCalibrated(unsigned int *sensor_values, unsigned char readMode)
{
    ...
    for(i=0;i<_numSensors;i++)
    {
		...
		// Ensure that the sensor value is not less than the Cal min as other wise this may cause issues with the following maths
		if (sensor_values[i]< calmin) 
			sensor_values[i] = calmin;
...
void QTRSensorsRC::readPrivate(unsigned int *sensor_values)
{
    ...
    for(i = 0; i < _numSensors; i++)
    {
		...
		//only need to disable the pullup if it is an arduino board
		#if not defined(__PIC32MX__)
